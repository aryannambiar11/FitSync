...
import LoadWorkoutPopup from "../components/LoadWorkoutPopup";
...
const RefinedPlanPage1Before = () => {
  ...
  const [isLoadWorkoutPopupOpen, setLoadWorkoutPopupOpen] = useState(false);
  ...
  const openLoadWorkoutPopup = useCallback(() => {
    setLoadWorkoutPopupOpen(true);
  }, []);

  const closeLoadWorkoutPopup = useCallback(() => {
    setLoadWorkoutPopupOpen(false);
  }, []);
  ...
  return (
    <>
      <div className="w-full relative h-[844px] text-left text-xl text-black font-poppins">
        ...
        <div className="absolute h-[4.62%] w-[48.97%] top-[11.02%] right-[10%] bottom-[84.36%] left-[41.03%]">
          <img
          ...
          />
          <TextField
            className="absolute h-[41.03%] w-[10.47%] top-[25.64%] right-[82.2%] bottom-[33.33%] left-[7.33%]"
            type="date"
        ...
        />
        <div
          className="absolute h-[3.91%] w-[38.21%] top-[60.78%] right-[50%] bottom-[35.31%] left-[11.79%] cursor-pointer text-center text-mini"
          onClick={openLoadWorkoutPopup}
        >
          <div className="absolute h-full w-full top-[0%] right-[0%] bottom-[0%] left-[0%] rounded-3xs bg-deepskyblue-200" />
          <b className="absolute h-[78.79%] w-[90.6%] top-[12.12%] left-[5.37%] inline-block text-transparent !bg-clip-text [background:linear-gradient(#fff,_#fff),_linear-gradient(#fff,_#fff),_linear-gradient(#fff,_#fff),_#fff] [-webkit-background-clip:text] [-webkit-text-fill-color:transparent]">
            Load Template
          </b>
        </div>
        <button
      ...
      {isLoadWorkoutPopupOpen && (
        <PortalPopup
          overlayColor="rgba(113, 113, 113, 0.3)"
          placement="Centered"
          onOutsideClick={closeLoadWorkoutPopup}
        >
          <LoadWorkoutPopup onClose={closeLoadWorkoutPopup} />
        </PortalPopup>
      )}
